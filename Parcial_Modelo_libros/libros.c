...
*
*/
int libros_buscarNombre(Libros array[], int size, char* valorBuscado, int* indice)                    //cambiar libros
{
    int retorno=-1;
    int i;
    if(array!=NULL && size>=0)
...
int libros_alta(Libros array[], int size, int* contadorID)                          //cambiar libros
{
    ...
    if(array!=NULL && size>0 && contadorID!=NULL)
    {
        ...
        else
        {
            ...
            printf("\n Posicion: %d\n ID: %d\n Id del Autor: %d\n Nombre del Libro: %s\n",
                   posicion, array[posicion].idUnico,array[posicion].idAutor,array[posicion].nombre);
            retorno=0;
        }
    }
    return retorno;
    ...
    if(array!=NULL && size>=0)
    {
        for(i=0;i<size;i++)
        {
            if(array[i].isEmpty==1)
                ...
                printf("\n ID: %d\n Id del Autor: %d\n Nombre del libro: %s",
                       array[i].idUnico,array[i].idAutor,array[i].nombre);      //cambiar todos
        }
        retorno=0;
    }
    return retorno;
